...
class model:
            ...
            for j in range(0, 48):
                ...
                for d in range(0, 3):
                    ...
                    self.residual[j][d][i] = self.allf(
                        goal[i], tree[i], v[i], l[i]
                    ) / self.allf(maxId.x, tree[i], v[i], l[i])
                    self.addId(j, 1 - d)
    ...
    def fmax(self, func, x0, args=(), **kwargs):
        return minimize(
            lambda x, tree, v, l: -func(x, tree, v, l),
            x0,
            args=args,
            options={"maxiter": 100},
            **kwargs
...
a = model()
a.training(
    np.array([[[1, 2, 3], [1, 2, 3], [1, 2, 3]], [[2, 3, 4], [2, 3, 4], [2, 3, 4]]]),
    np.array([[1, 2, 3], [2, 3, 4]]),
    np.array([[3, 2, 1], [4, 3, 2]]),
    np.array([[1, 2, 3], [2, 3, 4]]),
)
print(a.worked(
